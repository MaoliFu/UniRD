...
if __name__ == '__main__':
    x = cv2.imread('C:/Users/hijack/Desktop/temp-rlt/test/25_0.png', cv2.IMREAD_COLOR)
    x = torch.from_numpy(x).permute(2, 0, 1).unsqueeze(0).float()
    # model = RandomizedQuantizationAugModule(8, transforms_like=False)
    jitter_trans = RandomizedQuantizationAugModule(8, transforms_like=False)
    # pil_x = PIL.Image.fromarray(x)
    ...
    for i in range(50):
        ...
        cv2.imwrite('C:/Users/hijack/Desktop/temp-rlt/25_0_rlt_{}.png'.format(i+50), y)
    print(y.shape)
