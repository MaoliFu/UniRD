...
def performances(fileinfos, rlts_dict, config):
    ret_metrics = {}
    ...
    for clsname in clsnames:
        ...
        masks_cls = rlts_dict[clsname]['mask']
        preds_cls = rlts_dict[clsname]['preds']
        file_cls = []
        for i in range(masks_cls.shape[0]):
            file_cls.append(clsname)
        ...
        if config.get("auc", None):
            for metric in config["auc"]:
                ...
                print("{}_{}_auc: {}".format(clsname, evalname, auc))
                # ret_metrics[clsname]={evalname}
    ...
    if config.get("auc", None):
        for metric in config["auc"]:
            ...
            print("{}_{}_auc".format("mean", evalname, mean_auc))
